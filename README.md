...
## Analysis
* 1. The execution time visibly increases as the graph grows from 30 to 100 vertices, showing that performance depends heavily on the graph size. The small graph with 10 vertices took longer only because of the initial Java JVM warmup
* 2. In my tests, DFS was consistently faster than BFS. This indicates that the recursive approach had less system overhead than managing a Queue object for BFS
* 3. Excluding the initial JVM warmup phase, the execution time scales proportionally with the graph size. This confirms the expected linear time complexity of O(V + E)
* 4. The structure of the graph directly dictates the traversal order. Because I connected each node to its immediate next nodes, BFS strictly visits nodes layer by layer, whereas DFS aggressively dives deep into the first available branch until it hits a dead end before backtrackingg
* 5. BFS is generally preferred when we need to find the shortest path in an unweighted graph, or when the target node is located close to the starting root
* 6. A major limitation of DFS is that it does not guarantee the shortest path. Additionally, it can get stuck in infinite loops if visited nodes are not properly tracked, and extremely deep graphs can cause a StackOverflow error due to recursion depth limits
